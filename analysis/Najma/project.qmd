...
## loop to create individual plots for 5 benchmark drugs

save_figures <- FALSE  # will be set to true later

purrr::walk(drugs, function(d) {
  
  p <- df_monthly_rate_tsibble |>
    filter(chemical == d) |>
    gg_season(rate_per_1000, labels = "right", labels_repel = TRUE) +
    labs(
      title = paste("Seasonal pattern:", d),
  ...
  print(p)
   ...
   if (save_figures) {
    ggsave(
      filename = here("outputs", "figures", paste0("gg_season_", d, ".png")),
...
purrr::walk(drugs, function(d) {
  ...
  if (save_figures) {
    ggsave(
      filename = here("outputs", "figures", paste0("gg_subseries_", d, ".png")),
      plot = p, height = 4, width = 8
    )
  }
})
