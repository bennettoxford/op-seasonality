...
DATA_DIR <- "data/"
PLOTS_DIR <- "outputs/nasir/unnormalised/figures"
TBL_DIR <- "outputs/nasir/unnormalised/tables"
...
detrend_stl <- function(ts, s.window = 13) {
  ...
  ts |>
    model(
      STL(total_items ~ trend() + season(window = s.window))
    ) |>
    components() |>
...
detrend_linear <- function(ts) {
  ...
  ts |>
    model(linear_trend = TSLM(total_items ~ trend())) |>
    residuals() |>
...
detrend_loess <- function(ts, span = 0.75) {
  ...
  ts |>
    ...
    mutate(
      ...
      trend_curve = loess(total_items ~ t, span = span)$fitted,
      detrended = total_items - trend_curve
    ) |>
    ungroup() |>
...
fit_models <- function(ts, df_periods, max_k = 6) {
  ...
  map_dfr(unique(df_periods$chemical), function(chem) {
    ...
    m0 <- ts_chem |> model(ARIMA(total_items ~ PDQ(0, 0, 0)))
    g0 <- glance(m0) |>
      mutate(
        .model = "K0",
        period = NA,
    ...
    models_df <- pmap_dfr(periods_chem, function(period, max_allowed_k) {
      ...
      for (k in 1:max_allowed_k) {
        form <- total_items ~ fourier(period = period, K = k) + PDQ(0, 0, 0)
