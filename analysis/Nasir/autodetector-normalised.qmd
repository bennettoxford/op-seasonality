...
visualise_ts <- function(ts, plot_title = NULL) {
  ...
  for (i in seq_along(chems)) {
    ...
    time_plot <- chem_ts |>
      ...
      theme_minimal() +
      labs(y = "Items per wd", x = "Month [1M]")

    seasonal_plot <- chem_ts |>
      gg_season(items_per_wd) +
      theme_minimal() +
      labs(y = "Items per wd", x = "Month [1M]")
...
detrend <- function(ts, w = 13) {
  ts |>
    model(
      STL(items_per_wd ~ trend() + season(), robust = TRUE)
    ) |>
      components()
}
